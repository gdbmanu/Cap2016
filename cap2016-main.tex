...
\subsection{Jeux de données de grande dimension}
\label{subsec:BPAE}
...
\begin{table}[h]
	\caption{Caractéristiques des cinq jeux jeux de données}
	\label{table:mce}
	\begin{center}
...
Le premier jeu de données, nommé SynSep, est un jeu de données synthétique linéairement séparable, de dimension 400. Il est constitué de 9 classes. Le nombre d'exemplaire généré est $10^5$.
La méthode de construction de ce jeu de données peut être trouvées dans \cite{kakade2008efficient}. L'idée est d'avoir une simulation simple de données de type document. Les différentes coordonnées représentent des termes d'un petit vocabulaire de taille $400$. 

Le second jeu de données, nommé SynNonSep,est construit de la même façon, à l'exception d'u bruit de label à 5\%, ce qui rend le jeu de données non-séparable. 
...
Les deux derniers jeux de données sont constitués de données réelles, de dimension plus faible, et avec un nombre d'exemplaires plus faibles. Nous comparons dans ce cas le comportement des algorithmes dans le cas de plongement des données dans des espaces de fonctions noyaux.  

Le quatrième jeu de données, nommé Pendigits, est un jeu de données réel créé pa E.Alpaydin and Fevzi.Alimoglu \cite{alimoglu1996combining,Alimoglu96methodsof}. 
Il contient des jeux de 250 échantillons d'écriture de 44 sujets. Chaque sujet doit écrire 250 chiffres dans un ordre aléatoire dans sur une tablete numérique de résolution 500 x 500. 
